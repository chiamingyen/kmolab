...
步驟一, 就是從 <http://www.ubuntu.com/download/server> 下載 Ubuntu Server 14.04 版本的 iso 檔案 (或者從近端 <http://140.130.17.17/public/ubuntu-14.04.3-server-amd64.iso> 下載), 然後燒錄到 DVD 光碟上, 然後用此光碟開機執行安裝.
...
步驟六, 利用 sudo apt-get install gcin, 安裝 gcin, 切換語系, 讓使用者可以利用各種輸入法輸入中文.
...
~~~script
acl cad_lab src 999.130.17.0/24
...
若在推送過程發現遠端有新的版本資料, 則必須透過 git pull 的方式拉下資料後, 嘗試進行自動合併.
